module Gluttonberg
  module Public
    class ArticlesController <   Gluttonberg::Public::BaseController
      ...
      def index
        @blog = Gluttonberg::Blog.published.where(:slug => params[:blog_id]).include(:articles).first
      ...
      def show
        @blog = Gluttonberg::Blog.published.where(:slug => params[:blog_id]).include(:articles).first
        if @blog.blank?
          @blog = Gluttonberg::Blog.published.where(:previous_slug => params[:blog_id]).include(:articles).first
